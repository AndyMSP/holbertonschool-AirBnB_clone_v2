...
from fabric.api import env, run, cd, put, runs_once, lcd, local
...
def do_clean(number=0):
    """ Keeps newest <number> releases and deletes the rest """
    ...
    with cd('/data/web_static/releases/'):
        result = run('echo *')
        dirs = result.split(' ')
        del_dirs = dirs[:number]
        del_str = ' '.join(del_dirs)
        run('rm -rf {}'.format(del_str))
